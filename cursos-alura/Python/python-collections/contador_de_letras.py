...
def analisa_frequencia_letras(texto):
    # trata o texto criando o dicionário
    quantidade_letras_texto1 = Counter(texto.lower())
    print(quantidade_letras_texto1)

    # exrtai o total de caracteres
    total_de_caracteres = sum(quantidade_letras_texto1.values())
    print(f'A quantidade de caracteres totais no texto 1 é : {total_de_caracteres}')

    # conta a frequência das palavras e extrai as mais comuns
    proporcoes= [(letra, round(frequencia/total_de_caracteres, 4)*100) for letra, frequencia in quantidade_letras_texto1.items()]
    proporcoes = Counter(dict(proporcoes))

    print(f'As mais comuns são: ')
